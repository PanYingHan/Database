...
def join_records():
    ...
    SELECT 
        ...
        CustomerOrder.product AS order_product, 
        CustomerOrder.quantity AS order_quantity, 
        CustomerOrder.amount AS order_amount
    FROM 
        Customer
    LEFT JOIN 
        Contact ON Customer.id = Contact.customer_id
    LEFT JOIN 
        CustomerOrder ON Customer.id = CustomerOrder.customer_id;
    """
    cursor.execute(query)
    joined_records = cursor.fetchall()
    return render_template('join.html', joined_records=joined_records)
